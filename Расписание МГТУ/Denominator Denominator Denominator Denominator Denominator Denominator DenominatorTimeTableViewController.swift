...
class DenominatorTimeTableViewController: UITableViewController {
    
    @IBOutlet var tableItems: UITableView!
    
    @IBAction func cancelToDenominatorTimeTableViewController(segue:UIStoryboardSegue) { }
    
    @IBAction func saveAddDenominator(segue:UIStoryboardSegue) {
        updateWeek(true)
    ...
    override func viewWillAppear(animated: Bool) {
        ...
        updateWeek(true) // загрузка данных для знаменателя
        tableView.reloadData()
    }
    ...
    override func numberOfSectionsInTableView(tableView: UITableView) -> Int {
        return sections.count // количнство секций в таблице
    }
    ...
    override func tableView(tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return Week[section].subjects[1].count // количество ячеек в секции
    }
    
    // заполнение каждой ячейки
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        
        let cell = tableItems.dequeueReusableCellWithIdentifier("denominatorTableCell")! as! SubjectTableCell
        
        let subject = Week[indexPath.section].subjects[1][indexPath.row] as Subject
    ...
    override func tableView(tableView: UITableView, commitEditingStyle editingStyle: UITableViewCellEditingStyle, forRowAtIndexPath indexPath: NSIndexPath) {
        ...
        if editingStyle == .Delete {
            ...
            do {
                let subj : Subject =  Week[indexPath.section].subjects[1][indexPath.row]
                
                let weekPredicate = NSPredicate(format:"week == %@", true);
    ...
    {
        // Проверка на переход
        if segue.identifier == "DSubjectDetail"
        {
            ...
            let subject = Week[indexPath!.section].subjects[1][indexPath!.row]
